...
//  Dispatcher
//
//  Created by Jorge Garcia on 09/09/20.
...
class AWSS3Manager {
    ...
    private init () { }
    let bucketName = "dev-is-testsiis-1409-s3-dispatcher-pdfreceipt" //5
    ...
    public func uploadfile(fileUrl: URL, fileName: String, contenType: String, progress: progressBlock?, completion: completionBlock?) {
    ...
    func test(fileUrl: URL, fileName: String, contenType: String, key:String){
       ...
       let uploadRequest = AWSS3TransferManagerUploadRequest()
        ...
        transferManager.upload(uploadRequest!).continueWith { (task) -> AnyObject? in
            ...
            if task.result != nil {
                let s3URL = NSURL(string: "http://s3.amazonaws.com/\(self.bucketName)/\(key)")!
                print("Uploaded to:\n\(s3URL)")
            }
            else {
